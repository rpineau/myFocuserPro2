...

{
    ...
    // Read in settings
    if (m_pIniUtil) {
        m_myFocuserPro2Controller.setPosLimit(m_pIniUtil->readInt(PARENT_KEY, POS_LIMIT, 0));
        m_myFocuserPro2Controller.enablePosLimit(m_pIniUtil->readInt(PARENT_KEY, POS_LIMIT_ENABLED, false));
    }
	m_myFocuserPro2Controller.SetSerxPointer(m_pSerX);
    m_myFocuserPro2Controller.setSleeper(m_pSleeper);
}
...
void X2Focuser::deviceInfoFirmwareVersion(BasicStringInterface& str)				
{
    if(!m_bLinked) {
        ...
        m_myFocuserPro2Controller.getFirmwareVersion(cFirmware, SERIAL_BUFFER_SIZE);
        str = cFirmware;
    }
}
...
int	X2Focuser::establishLink(void)
{
    ...
    nErr = m_myFocuserPro2Controller.Connect(szPort);
    if(nErr)
        m_bLinked = false;
    else
        m_bLinked = true;
...
int	X2Focuser::terminateLink(void)
{
    ...
    X2MutexLocker ml(GetMutex());
    m_myFocuserPro2Controller.haltFocuser();
    m_myFocuserPro2Controller.Disconnect();
    m_bLinked = false;
...
int	X2Focuser::execModalSettingsDialog(void)
{
    ...
    if ((nErr = ui->loadUserInterface("myFocuserPro2.ui", deviceType(), m_nPrivateMulitInstanceIndex)))
        return nErr;
    ...
    if(m_bLinked) {
        ...
        nErr = m_myFocuserPro2Controller.getPosition(nPosition);
        if(nErr)
            return nErr;
	...
	dx->setPropertyInt("posLimit", "value", m_myFocuserPro2Controller.getPosLimit());
	if(m_myFocuserPro2Controller.isPosLimitEnabled())
    ...
    if (bPressedOK) {
        ...
        if(bLimitEnabled && nPosLimit>0) { // a position limit of 0 doesn't make sense :)
            m_myFocuserPro2Controller.setPosLimit(nPosLimit);
            m_myFocuserPro2Controller.enablePosLimit(bLimitEnabled);
        } else {
            m_myFocuserPro2Controller.enablePosLimit(false);
        }
...
void X2Focuser::uiEvent(X2GUIExchangeInterface* uiex, const char* pszEvent)
{
    ...
    if (!strcmp(pszEvent, "on_pushButton_clicked")) {
        ...
        nErr = m_myFocuserPro2Controller.syncMotorPosition(nTmpVal);
        if(nErr) {
...
int	X2Focuser::focMaximumLimit(int& nPosLimit)			
{
	...
	X2MutexLocker ml(GetMutex());
    if(m_myFocuserPro2Controller.isPosLimitEnabled()) {
        nPosLimit = m_myFocuserPro2Controller.getPosLimit();
    }
	else {
		nPosLimit = 100000;
	}
...
{   int nErr;
    ...
    X2MutexLocker ml(GetMutex());
    nErr = m_myFocuserPro2Controller.haltFocuser();
    return nErr;
}
...
int	X2Focuser::startFocGoto(const int& nRelativeOffset)	
{
    ...
    X2MutexLocker ml(GetMutex());
    m_myFocuserPro2Controller.moveRelativeToPosision(nRelativeOffset);
    return SB_OK;
}
...
int	X2Focuser::isCompleteFocGoto(bool& bComplete) const
{
	...
	nErr = pMe->m_myFocuserPro2Controller.isGoToComplete(bComplete);
...
int	X2Focuser::endFocGoto(void)
{
    ...
    X2MutexLocker ml(GetMutex());
    nErr = m_myFocuserPro2Controller.getPosition(m_nPosition);
    return nErr;
}
...
int X2Focuser::focTemperature(double &dTemperature)
{
    ...
    if(timer.GetElapsedSeconds() > 30.0f || m_fLastTemp < -99.0f) {
        nErr = m_myFocuserPro2Controller.getTemperature(m_fLastTemp);
        timer.Reset();
    }
